...
- كما أضيفت مراجعة نصية أولية أقرب إلى JAWS:
  - `Insert+Tab` لقراءة معلومات موسعة عن العنصر الحالي
  - تكرار `Insert+Tab` بسرعة لطبقة تشخيصية أعمق تشمل الإطار والحالة والأنماط المدعومة
  - `Insert+Home` لملخص النافذة الحالية
  - `Insert+End` لملخص حالة العنصر الحالي
  - `Insert+Enter` لتبديل `Text Review`
  - الأسهم للحرف والسطر داخل هذا الوضع
