...
- وضع مراجعة نصية أولي يحاكي JAWS:
  ...
  - داخل وضع المراجعة: الأسهم الرئيسية أو أسهم لوحة الأرقام مع `NumLock` مغلق للحرف والسطر و`Ctrl+الأسهم` للكلمة والفقرة و`Home/End` لبداية ونهاية السطر، مع محاولة مزامنة المؤشر النصي
...
محرك Lua يحمّل السكربتات بهذا الترتيب، وكل طبقة لاحقة تستطيع override للتي قبلها:

1. `scripts/focus_profile.lua`
2. `scripts/apps/<process>.lua`
3. `scripts/user/focus_profile.lua`
4. `scripts/user/apps/<process>.lua`
5. `%LocalAppData%\\Lumina\\scripts\\focus_profile.lua`
6. `%LocalAppData%\\Lumina\\scripts\\apps\\<process>.lua`

بهذا يبقى السكربت الأصلي جزءًا من المشروع، بينما يمكن للمستخدم تخصيص السلوك بدون تعديل الملفات المدمجة.

## ترميز سكربتات Lua

- يجب حفظ كل ملفات `Lua` داخل `scripts/` وأي overrides مستقبلية بترميز `UTF-8`.
- يفضّل استخدام `UTF-8` بدون `BOM` لتفادي أي اختلافات بين المحررات والمحركات.
- محرك Lumina يحمّل السكربتات نصيًا على أساس `UTF-8` صريح، لذلك أي ملف محفوظ بترميز مختلف قد ينتج نطقًا مشوهًا أو تحذيرات في `logs/lumina.log`.
- ملف [.editorconfig](/d:/flutterProjects/lumina/.editorconfig) يحدد هذا افتراضيًا لملفات `*.lua`.

## سجلات التشخيص

- `inspector/focus-events.jsonl` لتتبع أحداث التركيز والنطق
- `logs/lumina.log` للسجل النصي العام
- `logs/errors.jsonl` للاستثناءات مع معلومات إضافية قابلة للمعالجة
...
- [docs/web-test-plan-ar.md](/d:/flutterProjects/lumina/docs/web-test-plan-ar.md)
